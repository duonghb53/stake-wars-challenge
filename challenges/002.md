...
lscpu | grep -P '(?=.*avx )(?=.*sse4.2 )(?=.*cx16 )(?=.*popcnt )' > /dev/null \
  ...
  || echo "Not supported"
```
![img](../images/check_cpu_supported.png)
...
![img](../images/go_back.png)

4 – Enter your wallet and press Enter

![img](../images/login_success.png)
...
```
near generate-key duonghb.factory.shardnet.near
```
duonghb ---> duonghb.factory.shardnet.near WHERE `duonghb` is you pool name

* Copy the file generated to shardnet folder:
Make sure to replace <pool_id> by your accountId
```
cp ~/.near-credentials/shardnet/duonghb.shardnet.near.json ~/.near/validator_key.json
```
* Edit “account_id” => duonghb.factory.shardnet.near, where `duonghb` is your PoolName
...
ExecStart=/home/root/nearcore/target/release/neard run
Restart=on-failure
RestartSec=30
KillSignal=SIGINT
TimeoutStopSec=45
